...
@section('content')
    ...
    <div x-data="{ open: false, image: '', showBack: false }" class="bg-gray-50 min-h-screen py-24 px-4 sm:px-6 lg:px-20 flex flex-col items-center space-y-16">
        ...
        <div class="max-w-4xl w-full bg-gradient-to-r from-blue-50 to-white rounded-3xl shadow-2xl p-8 transform hover:scale-105 transition-transform duration-300">
            {{-- صور العملة --}}
            @if($coin->image || $coin->back_image)
                <div class="w-full mb-4 flex justify-center gap-4">
                    @if($coin->image)
                        <button @click="image='{{ asset('public/storage/' . $coin->image) }}'; open = true; showBack = false"
                                class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">
                            الوجه
                        </button>
                    @endif
                    @if($coin->back_image)
                        <button @click="image='{{ asset('public/storage/' . $coin->back_image) }}'; open = true; showBack = true"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @foreach($coin->relatedCoins as $related)
                        <div class="bg-white rounded-3xl shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition duration-300 overflow-hidden group">
                            <button @click="image='{{ asset('public/storage/' . $related->image) }}'; open = true">
                                @if($related->image)
