    ...
    <div x-data="{ open: false, image: '' }"
        ...
        {{-- بطاقة العملة الرئيسية --}}
                <div class="max-w-4xl w-full bg-gradient-to-r from-blue-50 to-white rounded-3xl shadow-2xl p-8 transform hover:scale-105 transition-transform duration-300">
                    ...
                    {{-- صورة الوجه --}}
                    @if($coin->image)
                        <button @click="open = true; image='{{ asset('storage/' . $coin->image) }}'" class="w-full">
                    ...
                    {{-- صورة الظهر --}}
                    @if($coin->back_image)
                        <button @click="open = true; image='{{ asset('storage/' . $coin->back_image) }}'" class="w-full">
                            <img src="{{ asset('storage/' . $coin->back_image) }}"
                                 alt="{{ $coin->title . ' - خلفية' }}"
                                 class="w-full h-96 object-cover rounded-3xl mb-6 shadow-xl hover:scale-105 transition-transform duration-300">
                        </button>
                    @endif
                ...
                <div x-show="open"
                    ...
                    <div @click.stop class="relative w-full max-w-4xl">
                        <img :src="image" class="w-full h-auto max-h-[80vh] object-contain rounded-xl shadow-2xl mx-auto">
                        <button @click="open = false"
                                class="absolute top-3 right-3 text-white text-4xl font-extrabold">&times;</button>
                    </div>
                </div>
