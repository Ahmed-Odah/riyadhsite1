<div class="bg-gray-50 min-h-screen py-20 px-6 sm:px-12 lg:px-24">
    ...
    {{-- القسم الأول: الكتب المتاحة --}}
    <h2 class="text-2xl font-bold text-gray-800 mb-8">📚 الكتب المتاحة</h2>
    <div class="max-w-full mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 px-4 lg:px-12 xl:px-20">
        @foreach($books->where('status', 'published') as $book)
    ...
    {{-- القسم الثاني: الكتب قيد الطباعة --}}
    <h2 class="text-2xl font-bold text-gray-800 mb-8 mt-20">🖨️ الكتب قيد الطباعة</h2>
    <div class="max-w-full mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 px-4 lg:px-12 xl:px-20">
        @foreach($books->where('status', 'printing') as $book)
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col">
                <div class="relative overflow-hidden rounded-t-xl">
                    <img
                        src="{{ $book->image ? asset('public/storage/' . $book->image) : asset('1.jpg') }}"
                ...
                <div class="p-5 flex flex-col flex-grow">
                    ...
                    <div class="mt-auto bg-gray-400 text-white text-sm font-medium py-2 rounded-md text-center">
                        قيد الطبع
                    </div>
                </div>
            </div>
