                ...
                @foreach($officials as $official)
                    <tr class="text-gray-800 text-right">
                        <td class="px-4 py-2 border-b">
                            ...
                            <form action="{{ route('official-edit-view', $official->id) }}">
                                @csrf
                                <button type="submit" class="text-blue-500 hover:underline">تعديل</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-b">{{ $official->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-2 border-b">
                            @if($official->image)
                                <img src="{{ asset('public/storage/' . $official->image) }}" class="h-16 w-16 object-cover rounded" alt="غلاف">
                            @else
                                لا يوجد
                            @endif
                        </td>
                        <td class="px-4 py-2 border-b">{{ Str::limit($official->description, 50) }}</td>
                        <td class="px-4 py-2 border-b">{{ $official->title }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
