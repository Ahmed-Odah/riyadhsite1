...
@section('content')
    <div class="p-6">
        <!-- زر إضافة صورة جديدة -->
        <div class="mb-6">
            <a href="{{ route('chamber-create-view') }}"
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 text-right border border-gray-300 align-middle">
                            <div class="flex gap-2 justify-end">
                                <form action="{{ route('chamber-delete', $chamber->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                    @csrf
                                    <button type="submit"
                                ...
                                <form action="{{ route('chamber-edit-view', $chamber->id) }}" method="GET">
                                    <button type="submit"
                        ...
                        <!-- تاريخ الإنشاء -->
                        <td class="px-4 py-3 text-right border border-gray-300 align-middle">
                            {{ $chamber->created_at->format('Y-m-d') }}
                        </td>

                        <!-- الصورة -->
                        <td class="px-4 py-3 text-center border border-gray-300 align-middle">
                            @if($chamber->image)
                                <img src="{{ asset('public/storage/' . $chamber->image) }}"
                        ...
                        <td class="px-4 py-3 text-right border border-gray-300 align-middle whitespace-normal break-words">
                            {{ Str::limit($chamber->description, 80) }}
                        </td>
