...
@section('content')
    <div class="p-6">
        <!-- زر إضافة صورة جديدة -->
        <div class="mb-6">
            <a href="{{ route('externalsession-create-view') }}"
        ...
        <div class="overflow-x-auto rounded-lg shadow border border-gray-300">
            <table class="min-w-full bg-white table-fixed border-collapse border border-gray-300">
                ...
                @foreach($externalsessions as $session)
                    <tr class="hover:bg-gray-50 transition">
                        <!-- العمليات -->
                        <td class="px-4 py-3 text-right border border-gray-300 align-middle">
                            <div class="flex gap-2 justify-end">
                                <form action="{{ route('externalsession-delete', $externalsessions->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                                    @csrf
                                    <button type="submit"
                                ...
                                <form action="{{ route('externalsession-edit-view', $session->id) }}" method="GET">
                                    <button type="submit"
                        ...
                        <td class="px-4 py-3 text-center border border-gray-300 align-middle">
                            @if($session->image)
                                <img src="{{ asset('public/storage/' . $session->image) }}"
