    ...
    <div class="flex justify-between items-center container mx-auto relative">
        ...
        <a href="" class="flex flex-col items-start gap-0">
            <div class="flex items-center gap-5 -ml-2 sm:-ml-4 md:-ml-6 lg:-ml-8 xl:-ml-10">
                ...
                <img src="{{ asset('/resha1.png') }}" alt="Logo" class="w-30">
            </div>
        </a>
        ...
        <div class="{{!request()->is('/') ? 'text-black' : 'text-white'}} hidden lg:flex flex-col md:flex-row items-center justify-center gap-8 md:static absolute w-full md:w-auto p-4 md:p-0 z-20 top-full left-0 md:top-auto md:left-auto"
            ...
            {{-- روابط ثابتة --}}
            <div><a href="{{route('books.index')}}" class="nav-link flex items-center">{{ __('site.books') }}</a></div>
            <div><a href="{{route('sumbook')}}" class="nav-link flex items-center">ملخصات كتب</a></div>
            ...
            <div><a href="{{route('paintings')}}" class="nav-link flex items-center">معرض اللوحات</a></div>
            <div><a href="{{route('coin')}}" class="nav-link flex items-center">عملات عالمية</a></div>
            <div><a href="{{route('blog')}}" class="nav-link flex items-center">المدونة</a></div>
...
<div id="menu"
    ...
    <div><a href="{{route('homepage')}}" class="nav-link">الرئيسية</a></div>
    <div><a href="/whous" class="nav-link">من آنا</a></div>
    <div><a href="{{route('books.index')}}" class="nav-link">{{ __('site.books') }}</a></div>
    <div><a href="{{route('paintings')}}" class="nav-link">معرض اللوحات</a></div>
    <div><a href="{{route('certificate')}}" class="nav-link">الشهادات والدورات</a></div>
    <div><a href="{{route('blog')}}" class="nav-link">المدونة</a></div>
    <div><a href="{{route('coin')}}" class="nav-link">العملات العالمية</a></div>
    <div><a href="{{route('channel')}}" class="nav-link">قناة اليوتيوب</a></div>
    <div><a href="{{route('channeltik')}}" class="nav-link">قناة التيك توك</a></div>
    <div>
        @if(\Illuminate\Support\Facades\Auth::check())
            <form action="{{route('logout')}}" method="POST">
                @csrf
...
<style>
    ...
    /* اللون الأبيض مع التمرير للشاشات الكبيرة فقط */
